...
"""
Usage: unnamed.py [-h] [-r rootdir] [-l count] pokegold.sym
...
if args.rootdir:
	for line in subprocess.Popen(['make', '-C', args.rootdir, '-s', '-p', 'DEBUG=1'],
		...
		if line.startswith('pokegold_obj :='):
			objects = line[len('pokegold_obj :='):].strip().split()
			break
	else:
...
print(f'Unnamed pokegold symbols: {len(symbols)} ({unnamed_percent:.2f}% complete)')
if not objects:
	for sym in symbols:
		print(sym)
	sys.exit()
