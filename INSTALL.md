...
If you have different projects that require different versions of `rgbds`, it might not be convenient to install rgbds 0.9.0 globally. Instead, you can put its files in a directory within pokegold, such as `pokegold/rgbds-0.9.0/`. Then specify it when you run `make`:

```bash
make RGBDS=rgbds-0.9.0/ gold
```

```bash
make RGBDS=rgbds-0.9.0/ silver
```
